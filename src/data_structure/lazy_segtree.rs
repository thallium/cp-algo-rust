...
impl<F: MapMonoid> LazySegtree<F> {
    /// Creates an array of length `n`. All the elements are initialized to the identity
    /// element.
...
impl<F: MapMonoid> From<Vec<<F::M as Monoid>::S>> for LazySegtree<F> {
    /// Creates an array initialized to `v`.
...
impl<F: MapMonoid> LazySegtree<F> {
    ...
    /// Returns `op(a[l], ..., a[r - 1])` if l < r, or identity element if l = r.
    ...
    /// Applies `a[i] = f(a[i])`, for all `i` in `[l, r)`.
...
/// Lazy segment tree using [ac-library-rs](https://github.com/rust-lang-ja/ac-library-rs)'s implementation.
/// See [this](https://atcoder.github.io/ac-library/production/document_en/lazysegtree.html) for the mathmatical 
